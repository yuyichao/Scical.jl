...
SCICAL_EXPORT double
genlaguerre(unsigned n, unsigned m, double x)
{
    return std::tr1::assoc_laguerre(n, m, x);
}

static SCICAL_INLINE long double
...
SCICAL_EXPORT double
harmonic_recoil(unsigned n1, unsigned n2, double eta)
{
    return harmonic_recoil(n1, n2, (long double)eta);
}

static constexpr unsigned _theta_n = 21;
typedef std::array<long double, _theta_n> theta_ary_t;

static SCICAL_INLINE theta_ary_t
make_thetas()
{
    theta_ary_t ary;
...
SCICAL_EXPORT double
harmonic_scatter(unsigned n1, unsigned n2, double eta, double theta0)
{
    return harmonic_scatter(n1, n2, (long double)eta, (long double)theta0);
}
